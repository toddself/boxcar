...
module.exports = function formElement (type, params, actions, options) {
  ...
  if (type === 'select' || type === 'checkbox') {
    triggers.onchange = actions.onchange
  } else {
    triggers.oninput = actions.oninput
  }

  if (type === 'text') {
    triggers.onload = (node) => node.setSelectionRange(node.value.length, node.value.length)
  }

  return elFn(params, triggers, options, type)
}
